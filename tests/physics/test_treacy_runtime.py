...
def test_treacy_runtime_gdd_error_decreases_with_beam_radius() -> None:
    ...
    for point in points:
        assert np.isfinite(point.abcdef_gdd_fs2)
        assert np.isfinite(point.abcdef_tod_fs3)
    for point in points[-4:]:
        assert np.sign(point.abcdef_gdd_fs2) == np.sign(point.analytic_gdd_fs2)
        assert np.sign(point.abcdef_tod_fs3) == np.sign(point.analytic_tod_fs3)
...
def test_treacy_runtime_mirror_leg_changes_abcdef_result_while_analytic_stays_fixed() -> None:
    ...
    abcdef_gdd = np.array([point.abcdef_gdd_fs2 for point in points], dtype=np.float64)
    gdd_errors = np.array([point.gdd_rel_error for point in points], dtype=np.float64)

    assert len(analytic_gdd) == 1
    assert np.max(np.abs(abcdef_gdd - abcdef_gdd[0])) > 1e2
