...
def _build_treacy_optics(
    *,
    ...
    n_passes: Literal[1, 2],
    immersion_refractive_index: float,
    gap_medium_refractive_index: float,
    center_wavelength_nm: float | None = None,
) -> tuple[FrameTransformCfg | FreeSpaceCfg | GratingCfg, ...]:
    g1_incidence_angle_deg = float(incidence_angle_deg)
    diffraction_angle_deg = (
        compute_grating_diffraction_angle_deg(
            line_density_lpmm=float(line_density_lpmm),
            incidence_angle_deg=g1_incidence_angle_deg,
            wavelength_nm=float(center_wavelength_nm),
            diffraction_order=int(diffraction_order),
        )
        if center_wavelength_nm is not None
        else g1_incidence_angle_deg
    )
    ...
    optics: list[FrameTransformCfg | FreeSpaceCfg | GratingCfg] = [
        ...
        GratingCfg(
            instance_name="g2",
            line_density_lpmm=float(line_density_lpmm),
            incidence_angle_deg=float(diffraction_angle_deg),
            diffraction_order=int(diffraction_order),
            immersion_refractive_index=float(immersion_refractive_index),
        ),
        FrameTransformCfg(
            instance_name="post_g2_frame",
            x_prime_scale=-1,
        ),
    ]
    if n_passes == 2:
        optics.extend(
            [
                ...
                FrameTransformCfg(
                    instance_name="post_g2_return_frame",
                    x_prime_scale=-1,
                ),
                FreeSpaceCfg(
                ...
                FrameTransformCfg(
                    instance_name="post_g1_return_frame",
                    x_prime_scale=-1,
                ),
            ]
        )
...
def treacy_compressor_preset(
    *,
    ...
    optics = _build_treacy_optics(
        ...
        n_passes=n_passes,
        immersion_refractive_index=float(immersion_refractive_index),
        gap_medium_refractive_index=float(gap_medium_refractive_index),
    )
